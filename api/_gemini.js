...
function formatGeminiError(error) {
  ...
  const isQuota = status === 429 || errMsg.toLowerCase().includes("quota") || errMsg.toLowerCase().includes("resource_exhausted") || errMsg.toLowerCase().includes("rate limit") || errMsg.toLowerCase().includes("rate_limit");
  if (isQuota) {
    return {
      status: 429,
  ...
  const isOverloaded = status === 503 || errMsg.includes("503") || status === "UNAVAILABLE" || errMsg.toLowerCase().includes("overloaded");
  if (isOverloaded) {
    return {
      status: 503,
