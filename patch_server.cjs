...
const chatStartIdx = code.indexOf('app.post("/api/chat"');
const generateStartIdx = code.indexOf('app.post("/api/generate-questions"');
if (chatStartIdx !== -1 && generateStartIdx !== -1) {
  const beforeChat = code.substring(0, chatStartIdx);
  const afterChat = code.substring(generateStartIdx);
...
Always prioritize accuracy, completeness, and clarity. Analyze the entire image before producing your answer.\`;
  ...
  `;
  fs.writeFileSync('server.ts', beforeChat + streamImplementation + afterChat);
}
